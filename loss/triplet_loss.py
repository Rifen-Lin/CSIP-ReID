...
class TripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False):
        ...
        if self.margin is not None:
            loss = self.ranking_loss(dist_an, dist_ap, y)  # Loss(input1,input2,y)=max(0,margin+input2−input1×y)
        else:
            loss = self.ranking_loss(dist_an - dist_ap, y)  # Loss(input,target)=log(1+exp(−y⋅input))
