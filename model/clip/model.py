...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x, attn_weights  # attn_weights应该是(B, num_heads, T, T)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3, out_channels=width, kernel_size=patch_size, stride=stride_size, bias=False
        )
    ...
    def forward(self, x: torch.Tensor, cv_emb=None):
        ...
        return x11, x12, xproj, attn_maps
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int):
    ...
    if vit:
        state_dict["visual.positional_embedding"] = resize_pos_embed(
            state_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution, w_resolution
        )
    else:  # RN50
