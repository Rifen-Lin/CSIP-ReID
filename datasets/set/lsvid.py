...
class LSVID(object):
    ...
    def __init__(self, args, sampling_step=48):
        self.args = args
        self._root = self.args.DATASETS.ROOT_DIR
        ...
        query_IDX = h5py.File(self.query_IDX_path, mode="r")["query"][0, :]
        ...
        train, num_train_tracklets, num_train_pids, num_train_imgs, num_train_cams, num_train_vids = self._process_data(
            track_train, json_path=self.split_train_json_path, relabel=True
        )

        train_dense, num_train_tracklets_dense, num_train_pids_dense, num_train_imgs_dense, _, _ = self._process_data(
            track_train, json_path=self.split_train_dense_json_path, relabel=True, sampling_step=sampling_step
        )

        query, num_query_tracklets, num_query_pids, num_query_imgs, _, _ = self._process_data(
            track_query, json_path=self.split_query_json_path, relabel=False
        )

        gallery, num_gallery_tracklets, num_gallery_pids, num_gallery_imgs, _, _ = self._process_data(
            track_gallery, json_path=self.split_gallery_json_path, relabel=False
        )
    ...
    def _process_data(self, meta_data, relabel=False, json_path=None, sampling_step=0):
        ...
        if relabel:
            pid2label = {int(pid): label for label, pid in enumerate(pid_list)}
        tracklets = []
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            ske_tracklet_path = osp.join(self._root, "skeleton_3d_hmr2.0", meta_data[tracklet_idx, 0]) + "*"
            ske_paths = glob.glob(ske_tracklet_path)
            ske_paths.sort()
            ...
            if sampling_step == 0:
                tracklets.append(((img_paths, ske_paths), pid, camid, 1))
            else:
                num_sampling = len(img_paths) // sampling_step
                for idx in range(num_sampling):
                    if idx == num_sampling - 1:
                        tracklets.append(
                            ((img_paths[idx * sampling_step :], ske_paths[idx * sampling_step :]), pid, camid, 1)
                        )
                    else:
                        tracklets.append(
                            (
                                (
                                    ...
                                    ske_paths[idx * sampling_step : (idx + 1) * sampling_step],
                                ),
                                pid,
                                camid,
                                1,
